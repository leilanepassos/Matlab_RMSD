...
%         temp_roms = netcdf.getVar(avg,it_r,[0 0 49 0],[656 732 1 1],'double');
        ...
        rmsd_sst7      = rmsd_sst7 + (sst_roms7 - mean_rey).^2;
        rmsd_sstT7(n0) = nanrmse(sst_roms7,mean_rey);
        
        % Apaga a media sst-Rey a SST dos experimentos
        clear mean_rey sst_roms5 sst_roms7
...
title('Exp: 005lei - Vento última camada sigma');
cblabel('Erro Médio Quadrático');
saveas(gcf,['~/RESULTADOS/COMPARACOES/007lei_005lei/005lei_vento_sigma.png'])

figure(2)
contourf(Xlon,Ylat,rmsd_sst7); shading flat; colorbar; grid on;
set(gca, 'YLim', [min(min(lat_rho)), max(max(lat_rho))]);
set(gca, 'XLim', [min(min(lon_rho)), max(max(lon_rho))]);
set(gca, 'CLim', [rmsd_sst_min_CCSM4, rmsd_sst_max_CCSM4]);
title('Exp: 007lei - Vento interpolado 10m');
cblabel('Erro Médio Quadrático');
saveas(gcf,['~/RESULTADOS/COMPARACOES/007lei_005lei/007lei_vento_10m.png'])

figure(3)
plot(time,rmsd_sstT5,'k');hold on;
plot(time,rmsd_sstT7,'r');
datetick('x','mmmyy');
...
legend('005lei-ventoSig','007lei-vento10m')
saveas(gcf,['~/RESULTADOS/COMPARACOES/007lei_005lei/RMSD.png'])

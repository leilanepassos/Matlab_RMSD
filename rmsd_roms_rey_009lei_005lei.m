...
%         temp_roms = netcdf.getVar(avg,it_r,[0 0 49 0],[656 732 1 1],'double');
        ...
        rmsd_sst9      = rmsd_sst9 + (sst_roms9 - mean_rey).^2;
        rmsd_sstT9(n0) = nanrmse(sst_roms9,mean_rey);
        
        % Apaga a media sst-Rey a SST dos experimentos
        clear mean_rey sst_roms5 sst_roms9
...
saveas(gcf,['~/RESULTADOS/COMPARACOES/009lei_005lei/005lei_OBCFAC_120.png'])

figure(2)
contourf(Xlon,Ylat,rmsd_sst9); shading flat; colorbar; grid on;
set(gca, 'YLim', [min(min(lat_rho)), max(max(lat_rho))]);
set(gca, 'XLim', [min(min(lon_rho)), max(max(lon_rho))]);
set(gca, 'CLim', [rmsd_sst_min_CCSM4, rmsd_sst_max_CCSM4]);
title('Exp: 009lei - OBCFAC=6');
cblabel('Erro Médio Quadrático');
saveas(gcf,['~/RESULTADOS/COMPARACOES/009lei_005lei/009lei_OBCFAC_6.png'])

figure(3)
% plot(time_CCSM4,rmsd_sstT_CCSM4,'b');hold on;
plot(time,rmsd_sstT5,'k');hold on;
plot(time,rmsd_sstT9,'r');
datetick('x','mmmyy');
...
legend('005lei-OBCFAC-120','009lei-OBCFAC-6')
saveas(gcf,['~/RESULTADOS/COMPARACOES/009lei_005lei/RMSD.png'])
